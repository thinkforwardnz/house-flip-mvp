...
const AIAnalysisSummary = ({ deal }: AIAnalysisSummaryProps) => {
  ...
  const getMarketAnalysisExplanation = () => {
    ...
    const arvExplanation = analysis.estimated_arv 
      ? `The estimated ARV of ${formatCurrency(analysis.estimated_arv)} was calculated by analyzing ${comparables?.length || 0} comparable properties in ${deal.suburb}, considering factors such as property size (${deal.bedrooms}br/${deal.bathrooms}ba), recent sale trends, and market conditions.`
      : "ARV calculation is in progress.";

    const marketTrend = analysis.market_trend 
      ? `Current market trend analysis indicates ${analysis.market_trend} conditions in the local area.`
      : "";

    const daysOnMarket = analysis.avg_days_on_market 
      ? ` Properties in this area typically sell within ${analysis.avg_days_on_market} days, indicating ${analysis.avg_days_on_market < 30 ? 'strong' : analysis.avg_days_on_market < 60 ? 'moderate' : 'slower'} market demand.`
      : "";

    const confidence = analysis.market_confidence 
      ? ` Our confidence in this valuation is ${analysis.market_confidence}%, based on the quality and quantity of comparable data available.`
      : "";
  ...
  const getRenovationExplanation = () => {
    ...
    const confidence = renovation.confidence_level 
      ? ` This estimate has ${renovation.confidence_level}% confidence based on visual property assessment and Wellington renovation cost benchmarks.`
      : "";
  ...
  const getRiskAnalysisNarrative = () => {
    ...
    const recommendations = risk.recommendations?.length 
      ? ` Recommended mitigation strategies: ${risk.recommendations.slice(0, 2).join(', ')}.`
      : "";
  ...
  const getFinalRecommendations = () => {
    ...
    const nextSteps = [
      "Verify renovation cost estimates with local contractors",
      "Conduct detailed property inspection focusing on structural elements",
      "Confirm market analysis with recent local sales",
      "Secure pre-approval for purchase and renovation financing"
    ];
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="text-right">
            <Badge className={`mb-2 ${
              overallConfidence >= 80 ? 'bg-green-100 text-green-800' :
              overallConfidence >= 60 ? 'bg-yellow-100 text-yellow-800' :
