...
const RenovationTab = ({ deal, formatCurrency, /* renovationEstimate, */ onDealUpdate }: RenovationTabProps) => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      {/* Cost Summary */}
      <div className="bg-gray-50 p-3 xs:p-4 md:p-6 rounded-xl">
        <h4 className="font-medium text-navy-dark mb-3 sm:mb-4 text-sm sm:text-base md:text-lg">Cost Summary</h4>
        <div className="space-y-2 sm:space-y-4">
          <div>
            <Label htmlFor="selected-reno-cost" className="text-xs sm:text-sm md:text-base">Selected Renovations Total</Label>
            <Input 
              id="selected-reno-cost"
              type="text" 
              value={formatCurrency(totalSelectedCost)} 
              readOnly
              className="mt-1 font-medium bg-gray-100 text-xs sm:text-sm md:text-base" 
            />
          </div>
          <div>
            <Label htmlFor="contingency" className="text-xs sm:text-sm md:text-base">Contingency Buffer (15%)</Label>
            <Input 
              id="contingency"
              type="text" 
              value={formatCurrency(Math.round(totalSelectedCost * 0.15))} 
              readOnly
              className="mt-1 bg-gray-100 text-xs sm:text-sm md:text-base" 
            />
          </div>
          <div className="p-2 sm:p-3 bg-green-50 rounded-lg">
            <p className="text-xs sm:text-xs md:text-sm text-green-700">Total with Contingency</p>
      ...
      {deal.renovation_analysis?.recommendations && (
        <div className="bg-orange-50 p-2 sm:p-3 md:p-4 rounded-xl">
          <h4 className="font-semibold text-orange-900 mb-2 sm:mb-3 text-sm sm:text-base md:text-lg">AI Renovation Recommendations</h4>
          <div className="space-y-1 sm:space-y-2">
            {deal.renovation_analysis.recommendations.map((rec, index) => (
              <div key={index} className="flex items-start gap-2">
                ...
                <p className="text-xs sm:text-xs md:text-sm text-orange-800">{rec}</p>
              </div>
            ))}
          </div>
        </div>
